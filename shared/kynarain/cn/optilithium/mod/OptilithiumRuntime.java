...
public final class OptilithiumRuntime {
	...
	public static synchronized void ensureSetup() {
		...
		if (FabricLoader.getInstance().getEnvironmentType() != EnvType.CLIENT) {
			System.out.println("[OptiLithium] Not a client environment, OptiFine will not be loaded");
			return;
		}
		...
		try {
			...
			System.out.println("[OptiLithium] Preparing OptiFine " + OptifineVersion.version
			...
			OptilithiumSetup.optifineRuntimeJar = runtime.remappedJar().toFile();
			...
			setupSucceeded = true;
			System.out.printf("[OptiLithium] Ready: %d patched classes taken over by Fabric Loader in %.1f seconds%n",
					patchedClassCount, (System.nanoTime() - start) * 1e-9);
		} catch (Throwable t) {
			failure = t;

			if (!OptilithiumError.hasError()) {
				OptilithiumError.setError(t, String.format("OptiLithium failed to load OptiFine:\n%s", String.valueOf(t.getMessage())));
			} else {
				OptilithiumError.logError(t);
			}

			System.err.println("[OptiLithium] Failed to set up OptiFine, the game will continue without it");
			t.printStackTrace();
		} finally {
