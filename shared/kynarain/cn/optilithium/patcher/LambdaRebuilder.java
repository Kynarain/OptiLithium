...
public class LambdaRebuilder implements IMappingProvider, Closeable {
	private static final boolean ALLOW_VAGUE_EQUIVALENCE = !Boolean.getBoolean("optilithium.exactOnly");
			...
			if (lostMethod == null) {
				if (gainedMethod == null) {
					...
					if (!Objects.equals(lost.getFullName(), gained.getFullName())) {
						System.err.println("[OptiLithium] Lambda pair in " + className + " has different names on the two"
	...
	public void close() throws IOException {
		if (unpairedLambdas > 0) {
			System.err.println("[OptiLithium] " + unpairedLambdas + " lambda pair(s) had to be skipped while rebuilding"
